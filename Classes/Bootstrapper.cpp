...
bool Bootstrapper::init()
{
	...
	std::unordered_map<int, std::vector<cocos2d::Vec3>> spawnPointPositions = getTeamSpawnPoints(rootNode);
	std::vector<CharacterView> characters = getCharacterViews();

	_battlefield = std::make_unique<Battlefield>(rootNode, spawnPointPositions);
...
{
	static const std::regex regex("Team([12])");
	std::smatch match;
	...
	CCLOGWARN("No team number found in the string");
	return -1;
}

std::unordered_map<int, std::vector<cocos2d::Vec3>> Bootstrapper::getTeamSpawnPoints(cocos2d::Node* node)
{
	std::unordered_map<int, std::vector<cocos2d::Vec3>> spawnPositionsByTeam;
